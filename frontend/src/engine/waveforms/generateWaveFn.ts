...
export function generatePWaveFn(simOptions: SimOptions): (t: number) => number {
  ...
  const leftAmp =
    sinus.status === 'LAE' ? -0.18 :
    sinus.status === 'RAE' ? 0.1 :
    rightAmp;
  ...
  return function (t: number): number {
    ...
    return (right + left) * amp * 0.5;
  };
}

export function generateQRSTFn(simOptions: SimOptions): (t: number) => number {
  ...
  const stHeight = waveform?.STheight ?? 0.02; // ← 明示化！
